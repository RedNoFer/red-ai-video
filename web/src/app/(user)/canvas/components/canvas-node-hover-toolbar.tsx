...
import { Download, Ellipsis, FolderPlus, GitBranch, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Pencil, Plus, RefreshCw, Settings2, Trash2, Upload, Video } from "lucide-react";
...
type CanvasNodeHoverToolbarProps = {
    ...
    onApplyToDrama?: (node: CanvasNodeData) => void;
...
export function CanvasNodeHoverToolbar({
    ...
    const canApplyToDrama = Boolean(onApplyToDrama && node.metadata?.sourceSurface === "drama" && node.metadata.dramaField && (hasImage || hasVideo));
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(canApplyToDrama ? [{ id: "applyDrama", title: "应用到短剧镜头", label: "回短剧", icon: <GitBranch className="size-4" />, onClick: () => onApplyToDrama?.(node) }] : []),
