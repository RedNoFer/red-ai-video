...
describe("Canvas Agent generation settings", () => {
    it("maps explicit image size and count while keeping quality model-bound", () => {
        const preferences = updateCanvasAgentGenerationPreferences({}, "image", { size: "1536x1024", quality: "high", count: 4 });

        expect(preferences).toEqual({ mode: "image", image: { size: "1536x1024", count: 4 } });
    });
    ...
    it("keeps the compact trigger readable without dropping the primary output values", () => {
        expect(canvasAgentCompactPreferenceSummary("image", { mode: "image", image: { size: "1:1", quality: "low", count: 4 } })).toBe("1:1 · 4K 高清 · 4张");
        ...
        expect(canvasAgentCompactPreferenceSummary("image", {})).toBe("1024×1024 · 4K 高清 · 1张");
        expect(canvasAgentCompactPreferenceSummary("video", {})).toBe("智能 · 5秒");
        expect(canvasAgentCompactPreferenceSummary("image", { mode: "image", image: { size: "1824x1024", quality: "high", count: 4 } })).toBe("1824×1024 · 4K 高清");
