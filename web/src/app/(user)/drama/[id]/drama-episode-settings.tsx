            ...
            <div className={`${embedded ? "space-y-3" : "mt-4 space-y-4"}`}>
                ...
                <div className="space-y-3 border-t border-border pt-3">
                    ...
                    <p className="text-[11px] leading-5 text-muted-foreground">
                        保存后会刷新锁定时间。生成制作包时，剧本 GPT
                        使用这里最新的方案；每镜时长只规定单个逻辑片段长度，逻辑片段数量和整集总时长必须由完整剧情、对白容量、动作节拍与反应留白推导，不能预设为固定段数。片段内剪辑只决定当前片段的帧段/硬切密度；30秒高密度模式固定承载 8—11 帧和 7—10
                        次切换，固定 4/5 帧仅在主动选择时生效。
                    </p>
                </div>
            </div>
