...
describe("Drama generation production workspace", () => {
    ...
    it("uses the locked episode resolution and does not expose a client video-model selector", async () => {
        ...
        expect(settingsSource).not.toContain("DRAMA_VIDEO_RESOLUTION_OPTIONS");
        expect(settingsSource).toContain("清晰度：");
        expect(settingsSource).toContain("生产方案仅由剧本 GPT 锁定");
        expect(settingsSource).toContain("本处仅展示当前方案");
        expect(settingsSource).toContain("保存本集信息");
        expect(settingsSource).not.toContain("锁定并保存设置");
