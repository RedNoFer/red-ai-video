...
describe("Drama generation production workspace", () => {
    ...
    it("uses the locked episode resolution and does not expose a client video-model selector", async () => {
        ...
        expect(settingsSource).toContain("savedLockAt || planDraft.lockedAt");
        ...
        expect(scriptSource).toContain("全局参数设置");
        expect(scriptSource).toContain("保存并生成制作包");
        expect(scriptSource).toContain("终止对话");
        expect(scriptSource).toContain('controlCreativeAgentRun(activeRunId, "cancel"');
        const saveOnlyHandler = scriptSource.slice(scriptSource.indexOf("const savePlan = async"), scriptSource.indexOf("const savePlanAndGeneratePackage = async"));
        ...
        expect(frameEditorSource).not.toContain("appendDramaImageReferenceBindings");
        expect(frameEditorSource).toContain("compileDramaFrameSupplierPrompt");
    ...
    it("keeps syncing visual tasks while the storyboard stage is mounted", async () => {
        ...
        expect(source).toContain("releaseUntrackedVisualQueue(currentShot)");
