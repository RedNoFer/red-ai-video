...
describe("drama script workspace layout", () => {
    it("keeps the scene structure panel fixed while scene jumps scroll the editor body", async () => {
        const [page, workspace, editor, sections, sourceImport, review, storyboardShotCard] = await Promise.all([
        ...
        expect(workspace).toContain("selectShotRef.current({ shot })");
        expect(workspace).toContain("selectShotRef.current({ scene: storyScene, shot: firstShot })");
        ...
        expect(sourceImport).toContain("下载制作包模板");
        ...
        expect(review).toContain("章节导览");
        expect(review).toContain("data-drama-production-archive-section");
        expect(review).not.toContain('iconPosition="end"');
        expect(storyboardShotCard).not.toContain('iconPosition="end"');
    });
});
