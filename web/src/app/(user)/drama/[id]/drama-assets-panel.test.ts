...
describe("drama asset image results", () => {
    it("uses the approved scene board reference as the spatial anchor", () => {
        ...
        expect(dramaSceneBoardReference({ id: "scene-one", name: "议事厅", description: "", primaryReferenceId: reference.id, sceneReferenceBoard: { layout: "3x3", referenceId: reference.id }, references: [reference] })).toEqual(reference);
