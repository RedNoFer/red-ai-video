...
describe("generationPreferenceSummary", () => {
    it("keeps image, video and audio settings readable in one compact label", () => {
        expect(generationPreferenceSummary("image", {})).toBe("1024×1024 · 4K 高清（模型绑定）");
        ...
        expect(generationPreferenceSummary("image", { image: { size: "1024x1536", quality: "low", count: 2 } })).toBe("1024×1536 · 4K 高清（模型绑定） · 2张");
