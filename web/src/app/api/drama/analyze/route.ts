        ...
        const messages = [
            {
                ...
                content:
                    phase === "visual"
                        ? `你是影视视觉导演和表演导演。输入内容已经由用户审核，必须严格保留每个 shotId、镜头数量、顺序、人物、场景、对白、旁白、原文和时长。为每个镜头补充图片提示词、视频提示词、起始/结束帧提示词、镜头运动、连续性、结构化人物表演计划、逐句对白表演、色彩灯光计划和 framePlan。生成前先根据场景资产完成现实可行的调度：场景有哪些座椅、长凳、地面、通道、门窗、车辆舱位或遮挡；每名实际出镜者在同一参照系下位于哪里、朝向哪里、看向谁或什么、身体如何被支撑、和其他出镜者怎样相对。坐姿必须落在可见的合理支撑面；多人必须写清左右/前后和视线关系；场景 backgroundNpcPolicy 为 auto 时按场景实际需要判断 NPC，required 时写合理数量、分布密度和群体行为结果，forbidden 时不得出现 NPC；NPC 只作为背景群像，不进入 characterCodes 或角色锚点；没有原文、资产或 NPC 策略依据的人物不得新增。表演必须写成可执行的外在行为：情绪目标、情绪起中止递进、眉眼嘴角下颌、视线、呼吸、身体反应、语气、停顿、重音和节奏；禁止只写“表情自然”“情绪丰富”等抽象词。对白和旁白可以不进入静态图片文字，但对白引发的表情、视线、手部/身体姿态、道具或环境变化必须进入对应 framePlan.frames 的 imagePrompt。每帧 imagePrompt 必须是独立可执行的静态画面，写明本帧可见状态并与上一帧有真实可见差异；禁止复制整镜头提示词后追加“起始状态/动作展开/关键变化/结果状态”等通用词。Agent 模式的 framePlan 必须提供 2-9 个按真实动作节点连续覆盖镜头时长的帧段，固定 4/5 帧只在项目主动选择时执行；发生视线转移、人物反应、空间揭示、道具变化或机位/景别/构图切换时必须有对应新帧。灯光必须明确色板、色温、主光、补光、轮廓光、反差、材质反射、肤色保护和跨镜继承/过渡。连续性必须明确景别、机位、构图、人物站位、视线、动作起止、屏幕运动方向和轴线规则。镜头之间要保持人物服装、道具、空间、表演状态和光色关系连续。不得新增输入中没有的剧情事实。必须调用 design_drama_visuals。不要使用 Markdown。${globalVisualRule}${schemaInstruction}`
                        : phase === "video_prompt"
