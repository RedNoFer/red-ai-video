...
export async function POST(request: Request) {
    ...
    try {
        ...
        const dialogueCapacityInstruction = hasTimedDialogue
        ...
        const messages = [
            {
                ...
                content:
                    phase === "visual"
                        ...
                        : phase === "video_prompt"
                          ? `你是图生视频执行提示词导演。本次只执行一次 canonical drama-video-director Skill：\n${DRAMA_VIDEO_PROMPT_DIRECTOR_RULES}${seedance25VideoInstructions ? `\n本次时长与供应商路由补充（只用于当前执行，不输出模式名）：\n${seedance25VideoInstructions}` : ""}仅根据输入的镜头事实、已验收帧、连续性状态和脱敏 referenceMaterials 直接生成完整公开视频提示词和内部 framePlan；不得生成图片提示词、改变镜头事实、输出 URL、内部 ID、供应商字段或解释文字。必须调用 generate_drama_video_prompts。公开视频使用小墨式简洁导演镜头卡：每个真实 framePlan 时间段输出一个“### 镜头 N | 时间范围 | 景别 | 焦段 | 机位角度 | 运镜方式 | 人物镜头/非人物镜头”卡片，并在卡片中填写场景、画面内容、光影、色调、台词、人声、音效；不要输出八段标题，也不要输出起点、动作与触发、可见衔接、终点等内部字段名。画面内容必须写屏幕上能看见的进行中动作和结果，台词必须使用“说话人说：“实际台词””格式；没有内容写“无”。参考图 alias、职责和供应商顺序由 referenceManifest 管理，公开正文不要自行编号。${DRAMA_DIALOGUE_TIMING_RULES}优化时必须逐句读取输入 shots.utterances 的 startSecond/endSecond、pauseBeforeSeconds/pauseAfterSeconds、speechRate 和 speechRateCharsPerSecond；对白不能被压进短于其可说时长的时间段，若当前时间段容纳不下，应保留动作节点并把对白放入足够长的连续时间段，不能通过异常加速解决。以下是服务端对白容量预检，必须逐条执行：\n${dialogueCapacityInstruction}${videoPerformanceInstruction}${videoOptimizationInstruction}${globalVisualInstruction}${videoReferenceInstruction}${schemaInstruction}`
                          : phase === "image_prompt"
