...
import { DramaProjectServiceError, updateDramaShotPromptForUser, updateDramaShotPromptPatchForUser } from "@/lib/server/drama-project-service";
...
export async function PATCH(request: Request, context: Context) {
    ...
    try {
        ...
        const compact = new URL(request.url).searchParams.get("response") === "shot";
        const input = parsed.data && typeof parsed.data === "object" ? (parsed.data as Record<string, unknown>) : {};
        if (compact) {
            const result = await updateDramaShotPromptPatchForUser(user.id, params.id, params.episodeId, params.shotId, parsed.data);
            return NextResponse.json({ code: 0, data: result, msg: "短剧视频提示词已更新" });
        }
