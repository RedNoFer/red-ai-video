...
const CREATIVE_ACTION_RE = /生成|制作|创作|设计|写|撰写|起草|改写|改成|换成|润色|优化|编辑|修改|调整|修复|重绘|配音|旁白|动起来|转视频|做(?:一|个|张|条|份|套|组)/u;
...
const PROJECT_REQUEST_RE = /创建|建立|整理成|画布|短剧|项目/u;
const CONTEXTUAL_CREATIVE_RE = /上一张|上一轮|参考|保持(?:人物|主体|构图)|继续(?:上一|刚才|前面|这个).{0,12}(?:图|画面|方案|创作|生成)/u;
...
export function isLikelyConversationPlannerPrompt(prompt: string) {
    const text = prompt.trim();
    if (!text) return true;
    return !CREATIVE_ACTION_RE.test(text) && !PROJECT_REQUEST_RE.test(text) && !CONTEXTUAL_CREATIVE_RE.test(text) && !IMAGE_RE.test(text) && !VIDEO_RE.test(text) && !AUDIO_RE.test(text) && !TEXT_RE.test(text);
}
