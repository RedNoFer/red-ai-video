...
const CONVERSATION_QUERY_RE = /[?？]|^(?:你好|您好|嗨|在吗)|^(?:hi|hello)\b|你是谁|你是什么|什么模型|能做什么|怎么用|如何使用|使用说明|帮助/iu;
...
export function isLikelyConversationPlannerPrompt(prompt: string) {
    ...
    if (!text) return true;
    return CONVERSATION_QUERY_RE.test(text) && !CREATIVE_ACTION_RE.test(text) && !IMAGE_RE.test(text) && !VIDEO_RE.test(text) && !AUDIO_RE.test(text) && !TEXT_RE.test(text);
}
