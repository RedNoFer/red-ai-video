...
export function agentPlannerSystemPrompt(surface: CreativeSurface, fallbackExample: string) {
    ...
    const dialogueProtocol =
        "所有生成型对话都遵循统一创作协议：先锁定目标、受众、用途和参考素材，再给一个明确推荐方向。图片 Prompt 按主体/身份锚点、当前变化、构图、光色材质、用途和约束组织；编辑必须分别写 change、preserve、constraints，且一次只改一个变量。短剧公开视频 Prompt 使用小墨 6.3 式简洁导演镜头卡：每个真实 framePlan 时间段对应一张“### 镜头 N | 时间范围 | 景别 | 焦段 | 机位角度 | 运镜方式 | 人物镜头/非人物镜头”卡片，正文写场景、画面内容、光影、色调、台词、人声和音效；内部起点、动作、衔接、终点和对白时间只保存在 framePlan 与质量门禁中，不强制公开八段标题或内部字段名。画面内容必须是可观察的进行中动作和结果，直接对白必须带中文引号且按自然语速连续分配；每镜保留一个有动机的主运镜，画幅先参与构图，不能把横向多人站位直接裁成竖幅。多资产只能使用稳定 assetId 绑定，@引用只表达用途，不能按标题或文本相似度猜测；内部规划规则、供应商字段和执行提示词不得进入公开消息。视频 Skill 只作为字段级质量层参与：补充动作因果、空间调度、摄影、表演、声音和连续性建议，并写回当前导演镜头卡与内部 framePlan 合同；不得生成第二套提示词、覆盖用户事实、改动素材顺序或把 Skill 原文附加到供应商请求。质量优先级固定为用户本轮事实、项目资产和已验收媒体、供应商协议、Skill 建议、默认风格。";
