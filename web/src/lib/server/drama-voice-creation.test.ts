...
describe("drama voice creation", () => {
    ...
    it("creates one idempotent Voice Design task without replacing the current voice early", async () => {
        ...
        expect(mocks.createTask).toHaveBeenCalledWith(
            expect.objectContaining({ config: expect.objectContaining({ voiceOperation: "voice-design", designPrompt: expect.stringContaining("Karin") }), voiceCreation: expect.objectContaining({ characterId: "character-one" }) }),
        );
    ...
    it("does not overwrite a role when a completed task has no voice_id or belongs to an old fingerprint", async () => {
        ...
        await expect(
            applyDramaVoiceCreationTask({ ...task("success"), voiceCreation: { projectId: "project-one", characterId: "character-one", fingerprint: "current" }, result: { url: "/api/reference-assets/preview.mp3", mimeType: "audio/mpeg" } } as never),
        ).resolves.toBeNull();
        expect(mocks.updateProject).not.toHaveBeenCalled();

        await expect(
            applyDramaVoiceCreationTask({
                ...task("success"),
                voiceCreation: { projectId: "project-one", characterId: "character-one", fingerprint: "old" },
                result: { url: "/api/reference-assets/preview.mp3", mimeType: "audio/mpeg", voiceId: "new-voice" },
            } as never),
        ).resolves.toBeNull();
        expect(mocks.updateProject).not.toHaveBeenCalled();
    });
    ...
    it("writes voice_id and local trial audio only when the active task fingerprint matches", async () => {
        const source = project("old-voice", { creationTaskId: "voice-task-one", creationFingerprint: "current" });
        mocks.project.mockResolvedValue(source);
        const updated = await applyDramaVoiceCreationTask({
            ...task("success"),
            voiceCreation: { projectId: "project-one", characterId: "character-one", fingerprint: "current" },
            result: { url: "/api/reference-assets/preview.mp3", mimeType: "audio/mpeg", assetId: "permanent/audio/preview.mp3", voiceId: "new-voice" },
        } as never);
    ...
    it("keeps Clone unavailable until the provider has a complete sample template", async () => {
        ...
        const source = project();
        await expect(
            createDramaVoiceCreationTask({ userId: "user-one", project: source, character: source.characters[0], mode: "clone", sampleAssetId: "permanent/audio/sample.mp3", requestId: "clone-one", origin: "https://vozeb.example" }),
        ).rejects.toThrow("未配置完整的样本音频请求模板");
        expect(mocks.createTask).not.toHaveBeenCalled();
    });
});
...
function settings(model = "voice-design", override: Record<string, unknown> = {}) {
    const config = {
        ...
        audioOperation: model === "voice-clone" ? ("voice-clone" as const) : ("voice-design" as const),
...
function project(voiceId = "", profile: Record<string, unknown> = {}): DramaProject {
    return {
        ...
        characters: [
            { id: "character-one", name: "Karin", description: "18岁男性，冷静克制", profile: { visualIdentity: "年轻男性", styling: "", colorPalette: "", consistencyRules: "" }, voiceProfile: { voiceId, speed: 1, instructions: "", ...profile } },
        ],
        scenes: [],
        props: [],
        clues: [],
...
function task(status: "pending" | "success" = "pending") {
    return {
        ...
        createdAt: 1,
        updatedAt: 1,
        config: { baseUrl: "/api/ai/system/channel-one", apiKey: "system", apiFormat: "openai" as const, model: "voice-design", logicalModel: "voice-design", channelId: "channel-one", advancedConfig: { protocol: "custom" } },
        prompt: "角色 Karin 的新声纹设计",
        candidateConfigs: [],
    };
}
