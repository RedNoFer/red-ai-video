...
productionBible.productionPlan.visual 必须包含具体的 visualStyle、artStyle 和 source（manual 或 agent）；productionPlan.video.shotDuration 只接受 15 或 30 秒用于剧本 Agent 逻辑镜头编排。framePolicy 为 fixed-4、fixed-5 时每镜必须分别输出 4 或 5 个帧段；framePolicy 为 agent 时由 Agent 按真实动作节点在 frameCountRange（默认 2-9）内智能切分并保留实际数量。customDirectorRules 是项目长期导演规则，执行优先级为本次用户补充 > 项目规则 > 项目事实 > 共享 Skill > 系统默认。
...
制作包正文禁止用通用阶段或质量占位句代替当前节点事实，包括“入口构图已建立”“动作展开”“关键变化”“结果状态”“动作节点已经成立”“主体的眉眼、呼吸、手部和道具接触关系清晰可见”“情绪通过身体动作呈现”等。可见表演状态必须写当前节点实际发生的具体眉眼、视线、呼吸、手部、身体或环境结果；历史包导入、规范化、序列化和生成前检查都必须重编译或拒绝这类文案。
同一镜头内的每帧图片必须独立使用固定角色、场景和道具锚点生成，不引用或等待同镜上一帧图片；只有跨镜头连续性边明确要求时，镜头第一帧才能使用上一镜当前视频版本经人工验收的实际尾帧。上一镜尾帧只承担入口身份、空间、光向和轴线连续，当前帧状态必须由本帧提示词独立给出。
...
场景资产硬规则：场景只生成一张高清、无人物、无文字的当前项目画幅单视角全景建立图，不生成九宫格、分格、方向标签或360°贴图；旧九宫格资产统一标记为 legacy-3x3，缺少 panorama 主基准图时必须重新生成并审核后才能生产。
...
制作包生成必须读取 productionPlan.video.shotDuration（仅允许 15 或 30 秒）作为逻辑镜头目标；同一场景且时间轴连续的 7s/8s 等碎片必须按目标时长合并，不能机械保留碎片；人物资产沿用项目已登记角色及基准图。固定 4/5 帧方案必须保存对应帧数；Agent 智能切分方案不得携带固定帧数，必须按真实动作节点在 2-9 个帧段（或项目 frameCountRange）内重新决定数量。参考图预算必须由同一时长契约驱动：15 秒及以下镜头的 productionPlan.references.maxImages 为 9，30 秒镜头为 30；30 秒包不得继续生成或导入旧的 9 张全局上限，供应商已声明的更小能力仍由提交阶段按真实能力拒绝。
...
framePlan.start.source 只能为 independent 或 previous_accepted_actual_tail；end.required 必须为布尔值；固定帧方案的 frames 必须为方案指定数量，Agent 智能切分方案的 frames 必须为 2-9 个真实动作节点（或项目明确的 frameCountRange），按 sequenceIndex 连续排列，从 0 秒无空白、无重叠覆盖完整镜头时长。每帧必须有稳定 id、startSecond、endSecond、actionPrompt、imagePrompt。
...
performancePlan 必须具体写情绪目标、情绪曲线、语气、节奏、呼吸、克制度，以及 start/middle/end 的情绪、面部动作、视线和身体/手部动作。情绪目标不得为空或只写“情绪自然/逐步变化/结果成立”等笼统标签；每个阶段必须引用当前镜头的实际人物、视线、呼吸、受力、道具或环境结果。dialoguePerformance 必须逐句写意图、语气、节奏、停顿、重音和对白前中后的面部反应；无对白只能为空数组。lightingPlan 必须写色板、色温、主光、补光、轮廓光、反差、材质反应、肤色保护、前镜继承和下镜过渡。
