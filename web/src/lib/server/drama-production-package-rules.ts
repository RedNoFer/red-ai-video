...
framePlan.start.source 只能是 independent 或 previous_accepted_actual_tail；framePlan.end.required 必须是布尔值；framePlan.referenceManifest 是参考图职责和顺序的唯一事实源，必须与镜头声明的角色、场景、道具和线索绑定，每张参考图只承担一个用途。framePolicy 为 agent 时执行唯一连续动作帧规则，按不可合并的冻结可见状态自适应提供 2–9 帧；固定策略仅在用户明确选择后执行。
