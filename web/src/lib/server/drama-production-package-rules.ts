...
productionBible.productionPlan.skills 至少包含 seedance-director / Seedance 导演 / 2.0。imagePrompt、startFramePrompt、endFramePrompt 只能描述一个静态画面：主体/身份锚点、当前可见姿态或道具状态、景别、构图、光线与必要约束；禁止写运镜、焦段、时间段、动作过程、对白或声音。图片编辑语义统一使用 change / preserve / constraints，change 只允许一个已定位变量，preserve 必须列出未修改的身份、构图或材质事实。videoPrompt 只写主体动态意图、一个主运镜、环境压力、可观察的表演变化（起始/中段/结束的眉眼、视线、呼吸、身体或手部动作）、结束状态和针对性约束；不得重复项目长档案、URL、画幅、清晰度、时长、参考图清单、逐帧时间线或全局规则，也不得包含“本内部镜头只执行”等内部说明。参考图编号和职责只由 referenceManifest 与服务端绑定块提供一次。角色名是项目资产事实，不得因为与 reference、ref 等英文缩写相似而改名、删除或当作内部占位符。
制作包生成必须读取 productionPlan.video.shotDuration（仅允许 15 或 30 秒）作为逻辑镜头目标；同一场景且时间轴连续的 7s/8s 等碎片必须按目标时长合并，不能机械保留碎片；人物资产沿用项目已登记角色及基准图。
framePlan.start.source 只能为 independent 或 previous_accepted_actual_tail；end.required 必须为布尔值；frames 必须为 1-9 个真实动作节点，按 sequenceIndex 连续排列，从 0 秒无空白、无重叠覆盖完整镜头时长。每帧必须有稳定 id、startSecond、endSecond、actionPrompt、imagePrompt。
