...
productionBible.productionPlan.visual 必须包含具体的 visualStyle、artStyle 和 source（manual 或 agent）；productionPlan.video.shotDuration 只接受 15 或 30 秒用于剧本 Agent 逻辑镜头编排。framePolicy 为 fixed-4、fixed-5 时每镜必须分别输出 4 或 5 个帧段；framePolicy 为 agent 时由 Agent 按真实动作节点智能切分为 1-9 个帧段并保留实际数量。
...
制作包生成必须读取 productionPlan.video.shotDuration（仅允许 15 或 30 秒）作为逻辑镜头目标；同一场景且时间轴连续的 7s/8s 等碎片必须按目标时长合并，不能机械保留碎片；人物资产沿用项目已登记角色及基准图。固定 4/5 帧方案必须保存对应帧数；Agent 智能切分方案不得携带固定帧数，必须按真实动作节点重新决定 1-9 个帧段。参考图预算必须由同一时长契约驱动：15 秒及以下镜头的 productionPlan.references.maxImages 为 9，30 秒镜头为 30；30 秒包不得继续生成或导入旧的 9 张全局上限，供应商已声明的更小能力仍由提交阶段按真实能力拒绝。
...
framePlan.start.source 只能为 independent 或 previous_accepted_actual_tail；end.required 必须为布尔值；固定帧方案的 frames 必须为方案指定数量，Agent 智能切分方案的 frames 必须为 1-9 个真实动作节点，按 sequenceIndex 连续排列，从 0 秒无空白、无重叠覆盖完整镜头时长。每帧必须有稳定 id、startSecond、endSecond、actionPrompt、imagePrompt。
