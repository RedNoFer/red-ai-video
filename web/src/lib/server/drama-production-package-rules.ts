...
productionBible.productionPlan.skills 至少包含 seedance-director / Seedance 导演 / 2.0 和 seedance-25-director / Seedance 2.5 导演 / 2.5。静态 imagePrompt、startFramePrompt、endFramePrompt 只执行 2.0 静态帧规则，只描述一个静态画面：主体/身份锚点、当前可见姿态或道具状态、景别、构图、光线与必要约束；禁止写运镜、焦段、时间段、动作过程、对白或声音。视频 videoPrompt 与第十一章分段视频 Prompt 同时执行 2.5 视频导演规则：15/20 秒使用普通视频阶段节拍，30 秒使用连续时间轴，用户明确续写、编辑、白模、转场或多宫格时使用对应模式；具体动作、触发、时间段起点、可见衔接和终点只进入 framePlan.frames，并由运行时和第十一章按时间顺序组装，禁止在两处重复。图片编辑语义统一使用 change / preserve / constraints，change 只允许一个已定位变量，preserve 必须列出未修改的身份、构图或材质事实。不得重复项目长档案、URL、画幅、清晰度、时长、参考图清单或全局规则，也不得包含“本内部镜头只执行”等内部说明。参考图编号和职责只由 referenceManifest 与服务端绑定块提供一次。角色名是项目资产事实，不得因为与 reference、ref 等英文缩写相似而改名、删除或当作内部占位符。
