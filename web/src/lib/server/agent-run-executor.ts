...
async function executeDramaScriptRun(run: AgentRun, origin: string, cookie: string, signal: AbortSignal) {
    ...
    const framePolicyInstruction =
        requestedFramePolicy === "agent"
            ? `帧数策略为 Agent 智能切分：每个镜头先列动作、表情/视线、道具、对手或 NPC 反应、空间揭示和摄影切换事件，再按真实节点在 ${requestedFrameRange?.min || 2}-${requestedFrameRange?.max || 9} 帧内决定连续帧段，必须保留实际帧数，不得套用固定模板。稳定场景用 2 帧，一个动作加反应用 3-4 帧，多人反应或摄影变化用 5-6 帧，复杂调度才用 7-9 帧。`
            : `帧数策略为 ${requestedFramePolicy}：每个镜头必须严格生成 ${requestedFrameCount} 个连续帧段。`;
    const shotFrameInstruction = requestedFramePolicy === "agent" ? `按真实动作节点智能切分为 ${requestedFrameRange?.min || 2}-${requestedFrameRange?.max || 9} 个连续帧段` : `固定生成 ${requestedFrameCount} 个连续帧段`;
    ...
    const effectiveInstruction = instruction
        ...
        .replace(
            /framePlan\.frames 默认必须是 (?:undefined|\d+) 个连续帧段/u,
            `framePlan.frames ${requestedFramePolicy === "agent" ? `必须按真实动作节点智能切分为 ${requestedFrameRange?.min || 2}-${requestedFrameRange?.max || 9} 个连续帧段` : `默认必须是 ${requestedFrameCount} 个连续帧段`}`,
        );
    const tool = {
