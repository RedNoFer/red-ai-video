...
async function executeDramaScriptRun(run: AgentRun, origin: string, cookie: string, signal: AbortSignal) {
    ...
    const lockedPlan = hasLockedPlan && normalizedSnapshotPlan
        ? {
    ...
    const effectiveInstruction = instruction
        ...
        .replace(/framePlan\.frames 默认必须是 (?:undefined|\d+) 个连续帧段/u, `framePlan.frames ${requestedFramePolicy === "agent" ? "必须按真实动作节点智能切分为 1-9 个连续帧段" : `默认必须是 ${requestedFrameCount} 个连续帧段`}`);
    const tool = {
            ...
            if (parsed.mode === "package") {
                ...
                if (lockedPlan) {
                    ...
                    const packageWithPlan = {
                        ...preview.package,
                        project: { ...preview.package.project, productionBible: { ...preview.package.project.productionBible, productionPlan: { ...lockedPlan, visual: mergedVisual } } },
                    };
