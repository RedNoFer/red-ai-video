...
async function executeDramaScriptRun(run: AgentRun, origin: string, cookie: string, signal: AbortSignal) {
    ...
    const lockedPlan = normalizedSnapshotPlan
        ? { ...normalizedSnapshotPlan, video: { ...normalizedSnapshotPlan.video, shotDuration: requestedShotDuration } }
        : undefined;
