...
async function executeDramaScriptRun(run: AgentRun, origin: string, cookie: string, signal: AbortSignal) {
    ...
    const requestedFrameCount = requestedFramePolicy === "fixed-4" ? 4 : requestedFramePolicy === "fixed-5" ? 5 : hasLockedPlan ? 5 : resolveDramaFrameCountPreference(run.prompt, 5);
    const lockedPlan = hasLockedPlan && normalizedSnapshotPlan
        ? {
              ...normalizedSnapshotPlan,
              video: { ...normalizedSnapshotPlan.video, shotDuration: requestedShotDuration, framePolicy: requestedFramePolicy, ...(requestedFrameCount ? { frameCount: requestedFrameCount } : {}) },
          }
        : undefined;
