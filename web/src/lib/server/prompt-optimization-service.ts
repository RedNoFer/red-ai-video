...
function promptOptimizationInstruction(mode: PromptOptimizationMode) {
    ...
    if (mode === "video")
        return `你是 VOZEB PRO 视频提示词编辑器。把用户原文改写为可直接发送的中文视频提示词，优先写清 0 秒入口、主体动作推进、一个有动机的主运镜、环境/声音反应和明确结束画面。${SEEDANCE_VIDEO_PROMPT_LAYOUT}${DRAMA_CONTINUOUS_FRAME_RULES}每个时间段都必须让姿态、表情/视线、手部/道具或环境产生可验证变化；减少“保持构图、主体稳定、情绪不变”等静态约束，只保留身份、空间、道具和轴线等必要连续性。保留用户的主体、人名、品牌、比例、时长、参考素材和否定要求，不新增剧情事实；只返回优化后的公开提示词，不解释修改过程，不输出内部规划、模型选择理由或思维链。`;
...
function parseOptimizedPrompt(value: string, mode: PromptOptimizationMode) {
    try {
        ...
        const prompt = typeof payload.optimizedPrompt === "string" ? formatPromptFieldLines(payload.optimizedPrompt, mode === "drama-frame" ? "static" : mode === "video" ? "video" : "static") : "";
