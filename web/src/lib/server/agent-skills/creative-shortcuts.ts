...
连续动作帧规则：每个镜头先拆成“建立场景 → 主体推进 → 关键动作 → 结果或反应/转场”的可见节拍，再为每个节拍生成一张独立冻结画面。静态帧不是无动作的氛围图：每帧必须呈现该节拍已经发生的姿态、表情/视线、手部或道具位置、环境状态中的至少一项，并且与上一帧形成肉眼可辨的状态差异。动作提示词写“发生了什么”，画面提示词写“这一刻已经变成什么样”；可见表演状态必须写出当前节点的具体眉眼、视线、呼吸、手部或身体结果，禁止使用“眉眼、呼吸、手部关系清晰可见”“情绪通过身体动作呈现”等通用句绕过关键点；不要让所有帧复用同一句场景描述后只添加“起始状态、动作展开、关键变化、结果状态”。
...
同一镜头内的关键帧只使用固定角色、场景和道具锚点，禁止引用或等待同镜上一帧图片；相邻帧必须有景别、机位或构图变化，不能整镜复用同一摄影视角。只有跨镜头连续性边明确要求时，下一镜第一帧才可使用上一镜当前视频版本经人工验收的实际尾帧；该尾帧只锁定入口身份、空间、光向和轴线，不能替代当前帧的独立可见状态。必要的连续性只锁定身份、服装、道具材质、空间轴线和光向；不得用“构图不变、主体稳定、情绪保持不变”等静态约束压住动作变化。时间段按实际动作节点和帧数连续分配，不预设固定秒数。`;
