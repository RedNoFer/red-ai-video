...
连续动作帧规则：每个镜头先拆成“建立场景 → 镜头/主体推进 → 关键动作 → 结果或反应/转场”的可见节拍，再为每个节拍生成一张冻结画面。静态帧不是无动作的氛围图：每帧必须呈现该节拍已经发生的姿态、表情/视线、手部或道具位置、环境状态中的至少一项，并且与上一帧形成肉眼可辨的状态差异。动作提示词写“发生了什么”，画面提示词写“这一刻已经变成什么样”；可见表演状态必须写出当前节点的具体眉眼、视线、呼吸、手部或身体结果，禁止使用“眉眼、呼吸、手部关系清晰可见”“情绪通过身体动作呈现”等通用句绕过关键点；不要让所有帧复用同一句场景描述后只添加“起始状态、动作展开、关键变化、结果状态”。
