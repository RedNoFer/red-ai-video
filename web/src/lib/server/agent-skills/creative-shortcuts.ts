...
连续动作帧规则：静态帧不是无动作的氛围图。Agent 模式先列动作、表情/视线、道具、对手或 NPC 反应、空间揭示和摄影切换事件，再按真实事件节点选择 2-9 帧；稳定进入/结果用 2 帧，一个主动作加反应用 3-4 帧，多人反应/空间揭示/明显摄影变化用 5-6 帧，复杂调度或多次揭示才用 7-9 帧。固定 4/5 帧只在用户主动选择时生效。每帧必须呈现该节点已经发生的姿态、眉眼/嘴角/下颌、视线、呼吸、手部或道具受力、身体重心、环境结果中的至少一项，并且与上一帧形成肉眼可辨的状态差异。动作提示词写“发生了什么”，画面提示词写“这一刻已经变成什么样”；可见表演状态必须写出当前节点的具体反应，禁止使用“眉眼、呼吸、手部关系清晰可见”“情绪通过身体动作呈现”“主体保持稳定”等通用句绕过关键点；不要让所有帧复用同一句场景描述后只添加“起始状态、动作展开、关键变化、结果状态”。
...
背景 NPC 规则：读取场景的 backgroundNpcPolicy。auto 由场景类型、空间容量、景别和剧情功能判断，required 必须安排合理数量范围、前中后景分布、密度和群体行为结果，forbidden 不得出现背景 NPC。NPC 只是不具名背景群像，不进入 characterCodes、角色锚点或独立角色资产；有明确剧情身份时才升级为正式角色。场景全景基准图始终无人，NPC 只写入镜头关键帧、视频时间段或构图参考；未配置时按 auto 判断，不得把空配置当作禁止 NPC。

同一镜头内的关键帧只使用固定角色、场景和道具锚点，禁止引用或等待同镜上一帧图片；相邻帧必须体现摄影或可见状态变化，不能整镜复用同一摄影视角和同一画面状态。只有跨镜头连续性边明确要求时，下一镜第一帧才可使用上一镜当前视频版本经人工验收的实际尾帧；该尾帧只锁定入口身份、空间、光向和轴线，不能替代当前帧的独立可见状态。必要的连续性只锁定身份、服装、道具材质、空间轴线和光向；不得用“构图不变、主体稳定、情绪保持不变”等静态约束压住动作变化。时间段按实际动作节点和帧数连续分配，不预设固定秒数。`;
...
export const SEEDANCE_STATIC_FRAME_RULES = `静态关键帧专用规则：这是单张冻结画面，不是完整视频提示词；不要添加时长、分时段、运镜过程、剪辑、对白或声音指令。输出前在内部完成一致性检查，但不得输出评估过程、思维链或检查清单。正向字段不能包含运镜、焦段、时间段、动作过程、对白或声音；“负面约束”字段可以明确写出无运镜过程、无对白、无声音指令等禁止项，校验时不能把负面字段误判为正向内容。检查景别与细节是否相容：ELS/极远景只能承载远景空间关系，若同时要求清晰面部、手部或道具细节，必须消除冲突并改用能承载这些细节的远景全身或中远景，不得同时保留互相矛盾的景别。检查多手、多肢体、多主体的数量、来源、位置和连接关系；用户明确需要的手或肢体必须具体化，禁止用与目标冲突的笼统否定覆盖它们。先检查场景现实性：人物姿势必须有可见且合理的支撑/接触结构，动作路径不能穿过座椅、墙体、车厢隔断或其他人物；多人必须以同一参照系写清左右/前后、朝向、视线和接触关系。读取场景 backgroundNpcPolicy：auto 按场景类型、空间容量、景别和剧情功能判断是否安排背景群像，required 必须写数量范围、密度、位置和行为结果，forbidden 不得出现 NPC；NPC 不进入 characterCodes 或角色锚点。场景全景基准图保持无人，NPC 仅出现在镜头关键帧和视频画面。封闭马车、船舱、车内、餐桌、病床等有明确功能结构的场景，优先让人物使用对应座位、长凳、床沿、通道或桌面，不能为了突出主体把人摆在不合场景常理的位置；没有原文、资产或 NPC 策略依据时不得添加陪同者。凡是“上一帧/上一镜/严格连续”的要求，必须有对应的真实参考图或结构化入口状态；没有证据时改写为当前镜头可执行的独立构图约束，不得假定未绑定素材。每张 @图片、@视频、@音频只承担一个明确用途，编号必须与本次请求的 references 顺序一致；禁止凭标题、内部 ID 或文本相似度猜测用途。引用节点与画面正文分层：有 referenceManifest 时保留每项引用用途，不自行生成不存在的 @编号；最终供应商提示词中的 @图片节点必须由服务端按实际 references 数组顺序追加，编辑态 imagePrompt 未内嵌节点不代表引用丢失。把 Ras、Ref、内部编号等模型不可理解的内部词改为语义化的画面约束，除非它们是用户要求出现的可见内容。压缩重复的身份、服装、材质和禁止项，按主体与可见状态、场景、构图、光线、风格、连续性、负面约束排序；保留用户事实、数量、比例、参考职责和画幅，不新增剧情事实。
