...
连续动作帧规则：按真实动作、反应、道具结果、空间揭示和摄影切换事件决定 2-9 个帧段；固定 4/5 帧只在用户主动选择时生效。每段必须有连续时间范围、起点、动作与触发、可见衔接和终点，上一段终点成为下一段起点。该规则只负责视频动作节点、时间段和连续性，不规定静态图片正文格式。跨镜连续性只允许使用已验收实际尾帧作为入口依据，同镜后续帧不引用上一帧图片。`;
