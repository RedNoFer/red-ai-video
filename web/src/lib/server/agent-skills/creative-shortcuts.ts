...
视频提示词默认导演质量层：不要只做同义改写。先从用户原文中提炼唯一戏剧职责、主体动作、被触发的对手/道具/环境反应和结束状态，再把它们转成可拍摄、可验收的公开视频提示词。只要内容包含对白、多个动作节点、人物反应或信息转折，默认按多镜头导演方案组织；镜头数量、时间段数量和切换次数按真实可见节拍自适应，不固定10段、9次切换或统一3/4段模板。需要切换时声明“镜头模式：内部切镜（N次）”，并在对应可见衔接中写出完整事件：“镜头事件：时间、类型、触发事件、新机位、切后主运镜、信息目的、承接”。只有用户明确要求单镜头，或内容确实只有一个连续动作时，才使用“镜头模式：连续镜头”。
...
单一主运镜：先声明镜头模式：连续镜头或镜头模式：内部切镜（N次）。关键帧和时间段只是状态锚点，不自动代表切镜。连续镜头每镜只保留一个有动机的景别/机位/运镜，并说明它响应的动作或信息变化；内部切镜时，每个切点必须在对应可见衔接中写出完整镜头事件：时间、类型、触发事件、新机位、切后主运镜、信息目的、承接，且切点必须落在 framePlan 段起点。镜头数量和切换次数按可见信息变化决定，不为满足固定配额而切镜。公共场景读取场景 NPC 策略：auto 按场景类型、空间容量、景别和剧情功能判断，required 必须安排合理数量范围、前中后景分布、密度和群体行为结果，forbidden 不得出现 NPC；NPC 仅是不具名背景群像，不加入主角色、锚点或独立资产，场景全景基准图保持无人。
