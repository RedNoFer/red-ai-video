...
连续动作帧规则：每个镜头先拆成“建立场景 → 镜头/主体推进 → 关键动作 → 结果或反应/转场”的可见节拍，再为每个节拍生成一张冻结画面。静态帧不是无动作的氛围图：每帧必须呈现该节拍已经发生的姿态、表情/视线、手部或道具位置、环境状态中的至少一项，并且与上一帧形成肉眼可辨的状态差异。动作提示词写“发生了什么”，画面提示词写“这一刻已经变成什么样”；不要让所有帧复用同一句场景描述后只添加“起始状态、动作展开、关键变化、结果状态”。必要的连续性只锁定身份、服装、道具材质、空间轴线和光向；不得用“构图不变、主体稳定、情绪保持不变”等静态约束压住动作变化。时间段按实际动作节点和帧数连续分配，不预设固定秒数。`;
...
全局设定：只写本镜头需要持续的环境、材质和主体不变量；不要把完整资产档案复制进来。
起始可见状态：只写首帧已经可见的主体、道具、空间和动作状态；若有上一镜实际尾帧，只继承该真实边界状态。
触发：导致主要动作发生的可见事件；没有明确触发时省略，不要编造剧情。
主体动作与反应：按“准备 → 接触/受力 → 结果 → 恢复”写一个可观察动作链；人物情绪用眉眼、嘴角、下颌、视线、呼吸、手部或身体反应表达。
阶段节拍：只有多事件或外部同步需要时才分段；每段独立一行，写动作变化和可检查的结束状态，时间范围连续、不重叠，不强行逐帧。
...
连续性锁：只保留身份、数量、服装、道具归属、空间方向、光向和轴线等高代价约束。
针对性约束：只列本镜头最可能且重做代价高的错误；不得添加内部 ID、URL、模型/API 参数或用户未提供的剧情事实。
规则：有参考素材时只继承其声明的属性；静态图生视频只描述运动和变化，不重复整张静态图；每镜只保留一个主要变化。每个非空字段必须独立一行，禁止把所有字段压成一段逗号串。`;
...
6. 站位与视线：主体站位、目光落点、前进空间。
7. 三层空间：前景、中景、背景分别写清各自承载的元素。
...
站位与视线：主体站位、目光落点和动作空间。
三层空间：前景、中景、背景分别写明承载的元素。
