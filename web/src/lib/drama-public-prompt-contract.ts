...
静态帧公开合同（Seedance 2.0）：静态关键帧、可见状态、可见表演状态、景别、机位与构图、站位与视线、三层空间、光色与风格、负面约束。每个非空字段必须独立成行；只描述一个已经冻结的可见画面，必须写出当前主体、姿态或道具/环境状态和具体表演结果。禁止运镜、焦段、时间段、动作过程、对白、声音、参考图职责、内部 ID、URL 或解释文字；参考素材用途只由 referenceManifest 和服务端绑定承载。`;
