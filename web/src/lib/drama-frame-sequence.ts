...
function hasVisibleStaticState(value: string) {
    return /(?:静立|站立|坐在|低头|抬头|抬眼|睁眼|闭眼|惊醒|苏醒|呼吸|肩膀|脚步|看向|对视|视线|姿态|下颌|眉眼|手部|手指|握住|扣住|按住|放在|停在|位于|面向|朝向|保持|器物|容器|道具|波纹|阴影|反光|打开|闭合|破损|裂纹|血印|屏息)/u.test(value);
}

function hasVisibleSpatialResult(value: string) {
    return /(?:前景|中景|背景|左侧|右侧|前方|后方|中央|同一侧|空间|环境|室内|户外|入口|出口|开口|桌面|桌沿|门窗|通道|座位|支撑面|平面|靠墙|柱列|同框|站在|坐在|位于|处于|落在|停在|靠近|贴着|朝向|面向|视线|看向|对视|接触|支撑|握住|扣住|按住|接住|抓住|放在|道具|在[^，。；\n]{1,18}(?:中|内|旁|前|后|上|下|边|侧))/u.test(
        value,
    );
}
