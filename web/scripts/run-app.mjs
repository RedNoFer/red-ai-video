...
const environment = resolveDevelopmentEnvironment({ webRoot, environment: { ...process.env, PORT: port } });
const runtime = generationRuntimeEnvironment({ environment, allowEphemeralToken: true });
const appEnvironment = { ...runtime.environment, NEXT_DIST_DIR: runtime.environment.NEXT_DIST_DIR?.trim() || ".next-dev" };
