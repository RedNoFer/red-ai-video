...
- [短剧] 修复场景历史参考图与候选图恢复：已成功但未写入资产引用的图片任务会在项目读取时补回候选，历史可用参考图不会因候选数组存在而消失；资产抽屉只自动恢复 `pending/running` 任务，不会把历史 `success` 任务误显示为无点击的生图进程。
- [短剧] 按导演 Skill 强化视频与片段帧质量：视频提示词必须包含具体表演、逐段动作和主运镜；多帧静态图按关系建立、动作细节、对话反应和结果构图产生可见差异，并保持背景结构清晰；制作包和 Agent 输出新增语义重复帧、空泛动作与缺少具体运镜校验。
