...
- [短剧/制作包] 将小墨个人分镜 Skill 6.3 简镜头卡、内部切镜、动作—触发—结果—声音闭环、角色/场景站位继承和参考素材连续性规则同步到制作包模板与运行时门禁。
- [短剧] 将对白时长校验收紧为逐句口型容量硬门禁，防止完整对白被压入不足的镜头窗口；句前/句后停顿改为独立边界校验。
- [制作包] 下载模板由规范源编译生成，并携带当前 13 章结构、门禁登记、契约和导演规则，避免后续重新下载时丢失预防规则。

## v0.0.17

- [短剧] 30 秒高密度镜头关键帧上限扩展至 11 帧，前端提示、序列校验和制作包容量规则保持一致。
- [短剧] 新增已完成或生成中镜头的“拉取最新视频”操作，复用原供应商任务并同步结果，不重复创建任务或扣费。

- [短剧/提示词] 统一制作包与视频提示词优化的八段式公开视频结构；逐段镜头必须对应 `framePlan`，对白必须保留说话人和中文引号实际台词，并在供应商提交前拦截旧式或异常 Prompt。
- [短剧] 修复镜头生成被“生产方案尚未锁定”误判为硬阻断的问题；方案锁定、导演质量、资产锚点和帧间差异提示仅作可确认风险，普通视频生成可直接进入确认弹窗和生产任务。
- [短剧] 修复场景全景图被“禁止生成九宫格”负面词误判为旧九宫格、导致场景图无法进入镜头引用的问题；资产设定显式保存不再与整项目自动保存竞态冲突。
- [短剧] 修复资产编辑器提示词清空后自动回填；保存提示词不再隐式调用 GPT，并改为单资产局部持久化，提示词-only 保存不会让关联镜头失效。
- [短剧] 修复资产候选生图关闭页面后状态丢失、任务锁解除和批量取消无效：重新打开会恢复仍在执行或已成功但尚未回写的原任务，状态确认期间禁止重复提交；无任务 ID 的批量项也会进入取消态，失败/取消项可重新排队。
- [图片] 图片尺寸预设改为精确 `WxH`，默认 `1024x1024`，并支持 `3840x2160`、`2160x3840` 等 4K 尺寸；Sub2API 请求保留用户选择的精确分辨率。
- [短剧] 短剧制作包文本 Agent 请求改为遵循逻辑模型绑定的请求超时配置，未配置时默认 3 分钟，并由 5–30 分钟边界保护长制作包响应。
- [短剧] 单帧和逐帧生图改为只保存当前镜头/选中帧的生成状态；视觉运行创建、确认、恢复和失败回写不再触发整份 `DramaProject` 请求，资产候选也使用单资产局部更新。
- [导演 Skill] 提炼动作分镜的主体/相机/环境运动协同、受力因果和可见结果规则，定向整合到视频时间段、`framePlan` 动作字段与制作包表演/连续性字段，并同步运行时编译产物和 Codex 镜像。

## v0.0.16

- [短剧] 统一 NPC 场景策略、Agent 自适应 2–9 帧关键帧、具体表演与镜头变化规则，制作包、图片帧、视频提示词和外部 Codex 工作单使用同一导演 Skill 契约。
- [提示词] 新增基于项目真实镜头上下文的图片帧优化接口；帧提示词保存改为软质量风险二次确认，负面约束不再误触发静态帧字段校验，并同步 `imagePrompt` 与 `supplierPrompt`。
- [短剧] 支持项目级导演定制规则和场景 `auto/required/forbidden` NPC 设置，外部 Agent 输出的九字段帧提示词可直接粘贴、保存并用于生成。

## v0.0.15

- [短剧] 修复 30 秒镜头被错误按 9 张参考图拦截的问题，15 秒及以下保持 9 张、30 秒允许 30 张，并保留供应商更小能力的真实限制。
- [制作包] 生成阶段补齐具体人物表演、情绪目标和阶段动作，导入前校验制作包完整性，避免导入后才暴露缺失字段。
- [导演 Skill] 运镜质量检查按不同运动类型去重，重复描述同一主运镜不再误报；同步项目 Skill、运行时编译产物与 Codex Skill。

## v0.0.14

- [短剧] 修复连续逐帧生图状态同步、场景基准图预览、30 秒片段参考图上限和生成中页面自动更新。
- [提示词] 将人物表演、情绪目标、角色引用和镜头质量项调整为可修订提示，保留真实素材、关键帧和连续性等硬校验。
- [Agent] 提示词优化会接收当前镜头的校验问题，并返回可编辑的最新提示词，不再把提示问题伪装成供应商失败。
- [稳定性] 补充上游失败终态、资产生成持久化和短剧生产预检回归测试及 30 秒制作包。

## v0.0.13

- [构建] 让图片资产与 Seedance Skill 编译器按项目格式配置生成运行时模块，避免测试前置编译再次破坏发布格式门禁。

## v0.0.12

- [构建] 统一 Web 工程现有格式基线，恢复发布工作流的全量格式门禁并允许主应用镜像继续构建。

## v0.0.11

- [构建] 让导演 Skill 编译器按项目 Prettier 配置生成运行时文件，避免新增导演规则在发布质量门禁中被误判为未格式化。

## v0.0.10

- [安全] 升级 Next.js、Sharp、Vitest 及相关传递依赖到已修复版本，使发布镜像重新通过依赖漏洞审计。

## v0.0.9

- [短剧] 新增独立视频导演层，统一增强制作包、静态片段帧和视频提示词的镜头、光影、纵深、表演、节奏与连续性设计。
- [质量] 将资产事实、可见状态和尾帧连续性保持为硬门禁，将光源动机、焦段、纵深、节奏和反模板化问题作为可修订警告。
- [Skill] 项目内置导演 Skill 支持构建时编译，并提供显式同步到外部 Codex Skill 目录的命令。

## v0.0.8

- [模型渠道] 修复严格渠道配置保存时的规范化与逻辑模型路由绑定，避免独立配置被错误合并或保存后失效。
- [视频] 支持通用不鸣视频模型的多参考图请求，同时保留已验证模型与能力档案的边界约束。
- [短剧] 收紧镜头提示词与生产契约，并将对白时长偏差作为可见提醒而非无必要的生成阻断。

## v0.0.7

- [短剧] 镜头生产预览允许按当前需求选择非固定参考素材，同时保留连续性实际尾帧和全量关键帧的必要约束；预检将可修订的画面提示词问题降为明确告警，生产任务只绑定用户确认的参考图。
- [制作包] 新增《三年之后》15 秒镜头制作包、确定性构建脚本和产品设计基线，便于按镜头复用连续帧、对白与视觉契约。
