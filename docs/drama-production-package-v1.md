...
- 相邻帧可以引用上一帧图片，但引用职责只限于身份、服装材质、场景空间、光向和轴线等结构连续性；上一帧的姿态、视线、手部/道具状态和环境结果不得被锁定或复制，当前帧 `imagePrompt` 对这些可见状态具有更高优先级。供应商请求必须同时写出“当前帧替换上一帧哪些状态”，否则该帧不得进入生成。
...
- 新建镜头或制作包未提供 Agent 帧段时，服务端不生成默认帧；必须由 Agent 提供合法的 1–9 个真实动作节点，否则阻止保存或生产。
